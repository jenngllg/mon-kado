...
def safe_point(record):
    ...
    # Timestamp is parsed by the caller when supervising; never copy arbitrary tags or URLs.
    return {"metric": metric, "value": value, "family": family, "phase": phase, "stage": stage}
...
def verdict(points, profile, infrastructure):
    ...
    return {"schemaVersion": 1, "profile": profile,
            "verdict": "passed" if success else ("failed" if complete else "incomplete"),
