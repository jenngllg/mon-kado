...
def redirect(connect=http.client.HTTPConnection):
    ...
    try:
        connection.request("GET", "/liveness", headers={"Host": API_HOST})
        response = connection.getresponse()
        ensure(response.status in (301, 308) and response.getheader("Location") == "https://" + API_HOST + "/liveness",
...
def check(client):
    ...
    for path in ("/liveness", "/readiness", "/openapi/v1.json", "/security/csrf-token"):
        ...
        if path in ("/liveness", "/readiness"):
            ...
            value = json.loads(body)
                   ...
                   "/api/v1/auth/sessions" in value.get("paths", {}) and "/api/v1/wishlists" in value["paths"],
    ...
    status, headers, _ = client.request("OPTIONS", "/api/v1/wishlists", headers={
        "Access-Control-Request-Method": "POST", "Access-Control-Request-Headers": "authorization,content-type"})
    ensure(status == 204 and headers.get("access-control-allow-origin") == ORIGIN and
           headers.get("access-control-allow-credentials") == "true", "CORS_SMOKE_FAILED")
    _, headers, _ = client.request("OPTIONS", "/api/v1/wishlists", headers={
